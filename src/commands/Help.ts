...
export default class PingCommand extends Command {
	...
	public async exec(message: Message) {
		...
		const helpEmbed: MessageEmbed = new MessageEmbed()
			...
			.setDescription('Below are all commands listed which are available to use. The default prefix is: ');
		...
		handler.modules.map((v: Command) => {
			const cat = v.category.filter((v: Command, k: string) => v.category.id == v.id && k != 'default');
			if (!commandsWithDescription.has(v.category.firstKey())) {
				commandsWithDescription.set(cat.size > 1 ? cat.keyArray()[cat.size - 1] : cat.firstKey(), [v]);
			} else {
				const categoryID = cat.size > 1 ? cat.keyArray()[cat.size - 1] : cat.firstKey();
